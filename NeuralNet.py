...
class NeuralNet:
    ...
    def train(self, activation_func="sigmoid", max_iterations=1000, learning_rate=0.05):
        for iteration in range(max_iterations):
            out = self.forward_pass(activation_func)
            error = 0.5 * np.power((out - self.y), 2)
            self.backward_pass(out, activation=activation_func)
    ...
    def predict(self, test=None, activation_func="sigmoid", header=True):
        #Test data set which was splitted in neuralnet
        self.X = self.X_test
        self.y = self.y_test
        out = self.forward_pass(activation_func)
        error = 0.5 * np.power((out - self.y), 2)
...
if __name__ == "__main__":
    neural_network = NeuralNet("car.csv")
    neural_network.train(activation_func = "sigmoid", max_iterations=10000, learning_rate=0.05)
    testError = neural_network.predict(activation_func = "sigmoid")
    print("Final Test Error is: " + str(testError))
